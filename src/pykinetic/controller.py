...
class Controller(object):
    r"""Controller for simulation runs and plotting
    ...
    def __getattr__(self, key):
        if key in (
            ...
            raise AttributeError(
                "'Controller' object has no attribute '" + key + "'"
            )
    ...
    def run(self):
        ...
        if self.solver is None or self.solution is None:
            raise Exception(
                "To run, a Controller must have a Solver and a Solution."
            )
        ...
        if self.output_style == 1:
            ...
            output_times = np.ones(
                (self.num_output_times + 1 - self.start_frame)
            )
        else:
    ...
    def file_format(self, value):
        if value.lower() == "petsc":
            self._io_module = __import__(
                "clawpack.petclaw.fileio.petsc",
                fromlist=["clawpack.petclaw.fileio"],
            )
            self._file_format = value
        else:
            self._io_module = __import__(
                "clawpack.pyclaw.fileio.%s" % value,
