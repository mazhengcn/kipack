...
class Solution(object):
    ...
    :Properties:
        ...
        parameters for all patch,s are also reachable although Solution does
        not check to see if these parameters are truly universal.
    ...
    :Initialization:
        ...
        The initialization of a Solution can happen one of these ways
            ...
            3. `args` is a variable number of keyword arguments that describes
                the location of a file to be read in to initialize the object

    :Examples:

        >>> import pykinetic as pyclaw
        >>> x = pykinetic.Dimension('x',0.,1.,100)
        >>> domain = pykinetic.Domain((x))
        >>> state = pykinetic.State(domain,3,2)
        >>> solution = pykinetic.Solution(state,domain)
    """
    ...
    def patch(self):
        ...
        if self.domain:
            ...
            raise Exception("This is homogeneous case! Patch is None.")
    ...
    #                 write_p=write_p,
    #             )
    #         msg = "Wrote out solution in format %s for time t=%s" % (
    #             form,
    #             self.t,
    #         )
    ...
    #     True if the file has been successfully read into the given solution
    #     or False otherwise.  Options is a dictionary of parameters that each
    ...
    #        ``default = './_output'``
    #      - *file_format* - (string) Format of the file, should match on of
    #        the modules inside of the io package.  ``default = 'ascii'``
