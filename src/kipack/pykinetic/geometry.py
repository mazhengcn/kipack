...
class Grid:
    ...
    :Dimension information:
        ...
        that name such as ``grid.x.num_cells`` which would access the x
        dimension's number of cells.
    ...
    A grid is usually constructed from a tuple of Dimension objects:
        ...
        >>> from pykinetic.geometry import Dimension, Grid
    ...
    A grid can be extended to higher dimensions using the add_dimension()
    method:
    ...
    **Coordinates:**
    ...
    We can get the x, y, and z-coordinate arrays of cell nodes and centers
    from the grid. Properties beginning with 'c' refer to the computational
    (unmapped) domain, while properties beginning with 'p' refer to the
    physical (mapped) domain.  For grids with no mapping, the two are
    identical. Also note the difference between 'center' and 'centers'.
...
class Dimension:
    ...
    >>> from pykinetic.geometry import Dimension
    ...
    def __init__(self, *arg):
        if len(arg) > 1:
            ...
            if not isinstance(geom, list) and not isinstance(geom, tuple):
                geom = [geom]
            if len(geom) == 0 or isinstance(geom[0], Dimension):
                self.patches = [Patch(geom)]
            elif isinstance(geom[0], Patch):
                self.patches = geom
