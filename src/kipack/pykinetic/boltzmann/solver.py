...
def default_tfluct(self):
    if self.tfluct_solver:
        raise Exception(
            "You set solver.tfluct_solver=True, but solver.tfluct has not \
                been set."
        )
    pass
...
class BoltzmannSolver(Solver):
    r"""
    Superclass for all Boltzmann solvers.
    ...
    .. attribute:: time_integrator
        ...
        Time integrator to be used. Currently implemented methods:
            ...
            - 'SSP33'  : 3rd-order strong stability preserving method of
                         Shu & Osher
            - 'SSP104' : 4th-order strong stability preserving method Ketcheson
            - 'SSPLMM32': 2nd-order strong stability preserving 3-step linear
                          multistep method, using Euler for starting values
            - 'SSPLMM43': 3rd-order strong stability preserving 4-step linear
                          multistep method using SSPRK22 for starting values
            - 'RK'     : Arbitrary Runge-Kutta method, specified by setting
                         `solver.a` and `solver.b` to the Butcher arrays of
                         the method.
            - 'LMM'    : Arbitrary linear multistep method, specified by
                        setting the coefficient arrays `solver.alpha` and
                        `solver.beta`.
    ...
    .. attribute:: tfluct_solver
        ...
        Whether a total fluctuation solver have to be used. If True the
        function that calculates the total fluctuation must be provided.
        ``Default = False``
    ...
    def __init__(self, riemann_solver=None, collision_operator=None):
        r"""
        Set default options for Boltzmann and call the
        super's __init__().
        """
        self.limiter = 1
        self.lim_type = -1
    ...
    def setup(self, solution):
        """
        Allocate RK stage arrays or previous step solutions.
        """
        if self.lim_type == 2:
            self.num_ghost = (self.weno_order + 1) // 2

        if self.lim_type == 2 and self.weno_order != 5:
            raise Exception(
                "Only 5th-order WENO reconstruction is implemented in Python \
                kernels."
            )
        ...
        try:
            if "SSPLMM" in self.time_integrator:
                if self.time_integrator == "SSPLMMk2":
                    ...
                    # accurate enough.
                    # For a smaller number of steps, higher values of
                    # cfl_desired and cfl_max can be used.
    ...
    def step(self, solution, take_one_step, tstart, tend):
        ...
        if self.accept_step is True:
            ...
            if self.time_integrator == "BEuler":
                self.dq_dt = self.dqdt(state) / self.dt
            else:
        ...
        # Runge-Kutta methods
        if self.time_integrator in ["Euler", "BEuler"]:
            ...
            for i in range(1, num_stages):
                ...
                # self._registers[i].q eventually stores dt*f(y_i) after stage
                # solution y_i is computed
        ...
        # Linear multistep methods
            ...
            if step_index < len(self._registers):
                ...
                # Update solution: alpha[-1] and beta[-1] correspond to
    ...
    def ssp104(self, state):
        ...
        if self.time_integrator == "SSP104":
            ...
            # Okay to copy state objects here since this only happens a few
            # times
            import copy
    ...
    def update_saved_values(self, state, step_index):
        r"""
        Updates lists of saved function evaluations, solution values, dt and
        dtFE for LMMs.
        For 3rd-order SSPLMM additional conditions are checked if
        self.check_lmm_cond is set to True.
        ...
        elif self.accept_step is True:  # Previous step was accepted
            if "SSPLMM" in self.time_integrator:
                cfl = self.cfl.get_cached_max()
                dtFE = self.dt / cfl * self.cfl_max / self.sspcoeff
    ...
    def check_3rd_ord_cond(self, state, step_index, dtFE):
        r"""
        This routine checks the additional conditions for the 3rd-order
        SSPLMMs.
        This is a posteriori check after a step is accepted.
        In particular, there is a condition on the step size for the starting
        values and a condition on the ratio of forward Euler step sizes at
        every step.
        If the conditions are violated we muct retrieve the previous solution
        and discard that step; otherwise the step is accepted.
        """
        lmm_cond = True
    ...
    def _set_mthlim(self):
        ...
        if len(self._mthlim) != self.num_waves:
            raise Exception(
                "Length of solver.limiters is not equal to 1 or to \
                solver.num_waves"
            )
    ...
    def dqdt(self, state):
        ...
        dq_dt += self.dq_collision(state)

        return dq_dt.flatten()

    def _allocate_registers(self, solution):
        r"""
        Instantiate State objects for Runge--Kutta stages and Linear Multistep
        method steps.
        ...
        If we create a MethodOfLinesSolver subclass, this should be moved
        there.
        """
        # Generally the number of registers for the starting method should be
        # at most equal to the number of registers of the LMM
        if self.time_integrator in ["Euler", "BEuler"]:
            nregisters = 0
        elif self.time_integrator == "SSP33":
            nregisters = 1
    ...
    def accept_reject_step(self, state):
        ...
        For SSPLMM32 the choice of step-size guarantees the cfl condition is
        satisfied for the steps the LMM
        is used. Hence, we need to check the cfl condition only for the
        starting steps.
        """
        accept_step = True
        cfl = self.cfl.get_cached_max()
