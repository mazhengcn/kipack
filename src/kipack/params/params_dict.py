...
class ParamsDict(object):
    ...
    def __init__(self, default_params=None, restrictions=None):
        ...
        Instantiate a ParamsDict given a set of default parameters and a list
        of restrictions. Upon initialization, it validates itself by checking
        all the defined restrictions, and raise error if it finds
        inconsistency.

        Args:
            default_params: a Python dict or another ParamsDict object
                including the default parameters to initialize.
            restrictions: a list of strings, which define a list of
                restrictions to ensure the consistency of different parameters
                internally. Each restriction string is defined as a binary
                relation with a set of operators, including {'==', '!=',  '<',
                '<=', '>', '>='}.
        """
        self._locked = False
        self._restrictions = []
    ...
    def override(self, override_params, is_strict=True):
        ...
        Args:
            override_params: a dict or a ParamsDict specifying the parameters
                to be overridden.
            is_strict: a boolean specifying whether override is strict or not.
                If True, keys in `override_params` must be present in the
                ParamsDict.
                If False, keys in `override_params` can be different from what
                is currently defined in the ParamsDict. In this case, the
                ParamsDict will be extended to include the new keys.
        """
        if self._locked:
            raise ValueError(
                "The ParamsDict has been locked. No change is allowed."
    ...
    def validate(self):
        ...
        This method validates the internal consistency using the pre-defined
        list of restrictions. A restriction is defined as a string which
        specfiies a binary operation. The supported binary operations are
        {'==', '!=', '<', '<=', '>', '>='}. Note that the meaning of these
        operators are consistent with the underlying Python immplementation.
        Users should make sure the define restrictions on their type make
        sense.
        ...
        Raises:
            ...
            ValueError: if the restriction defined in the string is not
                supported.
        """
        ...
        for restriction in self._restrictions:
            if "==" in restriction:
                ...
                if left_v != right_v:
                    raise KeyError(
                        "Found inconsistncy between key `{}` and key `{}`.".format(  # noqa
                            tokens[0], tokens[1]
                        )
                    )
            elif "!=" in restriction:
...
def nested_csv_str_to_json_str(csv_str):
    ...
    Spacing between commas and = is supported (e.g. there is no difference
    between "a=1,b=2", "a = 1, b = 2", or "a=1, b=2") but there should be no
    spaces before keys or after values (e.g. " a=1,b=2" and "a=1,b=2 " are not
    supported).
...
def override_params_dict(params, dict_or_string_or_yaml_file, is_strict):
    ...
    if isinstance(dict_or_string_or_yaml_file, dict):
        ...
        if isinstance(params_dict, dict):
            ...
            with fopen(dict_or_string_or_yaml_file) as f:
